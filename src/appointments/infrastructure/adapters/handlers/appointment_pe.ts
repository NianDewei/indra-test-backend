...
export const handler = async (event: SQSEvent): Promise<void> => {
    console.log('Processing PE appointments');
    
    // Initialize country-specific services using the factory pattern
    const scheduleRepository = CountryServiceFactory.createMySQLScheduleRepository('PE');
    const eventBusService = CountryServiceFactory.createEventBridgeService();
    const mysqlAppointmentRepository = CountryServiceFactory.createMySQLAppointmentRepository('PE');
